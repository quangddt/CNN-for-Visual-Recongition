...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    prob = np.exp(scores) / np.exp(scores).sum(axis=1, keepdims=1)
    ...
    grads['b2'] = I.sum(axis=0)
    grads['b2'] /= N 
    
    grads['W1'] = X.T.dot(I.dot(W2.T)*(V > 0).astype(np.float64)) 
    grads['W1'] /= N
    grads['W1'] += 2 * reg * W1
    
    
    grads['b1'] = (I.dot(W2.T) * (V > 0).astype(np.float64)).sum(axis=0)
    grads['b1'] /= N
